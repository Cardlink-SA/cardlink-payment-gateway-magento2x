...
/**
 * POST /api/sign-data — signs the 3D-Secure MPI form data for Apple Pay.
 *
 ...
 *
 * Magento route: cardlink_checkout/payment/applepaysigndata
 *
 * @author Cardlink S.A.
 */
class ApplePaySignData extends Action implements CsrfAwareActionInterface
{
    /**
     * @var Logger
     */
    ...
    public function execute()
    {
        ...
        try {
            ...
            $mid = $data['merchantID'] ?? $this->dataHelper->getApplePayMerchantId();
            ...
            $mpiKey = $this->dataHelper->getApplePayMpiPrivateKey();
            $secret = $this->dataHelper->getApplePaySharedSecret();
            ...
            if (strpos($signature, 'Error') === 0) {
                $this->logger->error('ApplePaySignData: ' . $signature);
            ...
            $this->logger->error('ApplePaySignData error: ' . $e->getMessage());
            $result->setHttpResponseCode(500);
     ...
     * @param string|null $mpiVersion MPI version to use in payload
